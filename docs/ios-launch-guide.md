...
**Fifth attempt (2026-09-22, EDIT no.96), FAILED AND REVERTED (EDIT no.98):** rewrote the long-press setup to use node-cloning (`cloneNode` + `replaceWith`) for guaranteed-clean re-attachment with zero duplicate-listener risk, plus a global `window.visualViewport` `resize` listener (debounced) that re-runs the setup automatically whenever the keyboard genuinely shows or hides anywhere in the app. The diagnostic toast confirmed this mechanism genuinely fires and re-attaches fresh listeners — yet the bug still persisted. This is significant: since fresh listener attachment is now proven (via direct evidence, not assumption) to happen and the bug still occurs, listener management is very likely **not** the actual cause at all. Fully reverted back to the clean baseline (verified via diff — zero difference in the moon long-press section specifically).

**Where this leaves us after five failed attempts across genuinely different mechanisms:** listener attachment, an error-banner overlay, keyboard blur at one transition point, and now keyboard blur/re-attachment at every transition point via the real platform signal — none of it. This strongly suggests the real cause is something more fundamental in how touch events reach this specific screen region during the broken state, which isn't something JS-side fixes can diagnose blindly. **Live Safari remote Web Inspector access (needs a Mac, even briefly) is now the clearly recommended next step** rather than further speculative fixes — it would show directly whether touch events are being intercepted by something else, whether an error is silently firing, or something in the WebView's own internal state, in a way that guessing from outside cannot.
